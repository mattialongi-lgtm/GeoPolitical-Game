...
export default function ExtractionDashboard({ user }: ExtractionDashboardProps) {
  ...
  return (
    <div className="space-y-5 max-w-2xl mx-auto pb-24">
      {/* Back */}
      <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm font-bold text-gray-400 hover:text-emerald-400 transition-colors">
      ...
      <div className="bg-gray-900/60 p-6 rounded-2xl border border-gray-800">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-gray-800/60 border border-gray-700/40">
            <ResourceIcon id="minerals" size={32} />
          </div>
          <div>
            <h1 className="text-2xl font-black text-white tracking-tight">Dashboard Risorse</h1>
            <p className="text-xs font-bold text-gray-400 uppercase mt-1">
              {dashboard.regionName} ({dashboard.regionId})
              {dashboard.isAutonomous && <span className="ml-2 text-[9px] font-black text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded-lg">🏛️ Autonoma</span>}
            </p>
      ...
      <div className="space-y-3">
        {(dashboard.resources || []).map((r: any) => {
          ...
          return (
            <div key={r.resourceType} className="bg-gray-900/60 p-5 rounded-2xl border border-gray-800 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gray-800/60 flex items-center justify-center border border-gray-700/40">
                ...
                <div className="text-right">
                  {r.deepActive && (
                    <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded-lg bg-purple-500/10 text-purple-400">
                      🔬 Deep Attiva
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-[9px] font-bold text-right text-gray-400">{usagePercent}% consumato</p>
              </div>
              {rt === 'gold_ore' ? (
                <div className="bg-amber-500/15 border border-amber-400/30 px-3 py-2 rounded-xl">
                  <p className="text-[10px] font-black text-amber-300">Solo l'oro restituisce Gold premium.</p>
                  <p className="text-[9px] font-bold text-amber-400">Base 30 per scavata a salute 1, con bonus crescente sulla salute della regione.</p>
                </div>
              ) : (
                ['oil', 'minerals', 'uranium', 'diamonds'].includes(rt) ? (
                  <div className="bg-gray-800/50 px-3 py-2 rounded-xl border border-gray-700/40">
                    <p className="text-[9px] font-bold text-gray-300">Risorsa estrattiva: consuma energia e non restituisce Gold premium.</p>
                  </div>
                ) : null
              )}
              ...
              {/* Player Experience */}
              {myExp > 0 && (
                <div className="bg-indigo-500/10 px-3 py-2 rounded-xl flex justify-between items-center">
                  <span className="text-[9px] font-black uppercase text-indigo-400">La tua EXP {label}</span>
                  <span className="text-xs font-black text-indigo-400">📊 {myExp}</span>
                </div>
              )}
            </div>
          );
        ...
        <div className="bg-gray-900/60 p-5 rounded-2xl border border-gray-800 space-y-3">
          ...
          <div className="space-y-1">
            {(dashboard.departmentBonuses || []).map((b: any) => (
              <div key={b.resourceType} className="flex items-center justify-between bg-gray-800/50 px-3 py-2 rounded-xl text-[10px] font-bold">
                <div className="flex items-center gap-2">
                  <ResourceIcon id={b.resourceType} size={14} />
                  <span className="text-gray-300">{RESOURCE_LABELS[b.resourceType as ResourceType] || b.resourceType}</span>
                </div>
                <span className="text-emerald-400">+{b.bonusLevel}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {playerExperience.length > 0 && (
        <div className="bg-gray-900/60 p-5 rounded-2xl border border-indigo-500/20 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {playerExperience.map((exp: any) => {
              ...
              return (
                <div key={exp.resourceType} className="bg-indigo-500/10 p-3 rounded-xl border border-indigo-500/20">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-gray-800/60 rounded-lg flex items-center justify-center border border-gray-700/40">
                      <ResourceIcon id={rt} size={18} />
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-indigo-400">{RESOURCE_LABELS[rt] || exp.resourceType}</p>
                      <p className="text-xs font-black text-white">EXP: {effective.toLocaleString()}{maxExp > 0 ? ` / ${maxExp.toLocaleString()}` : ''}</p>
                      {maxExp > 0 && (
                        <div className="w-full bg-indigo-500/20 h-1.5 rounded-full overflow-hidden mt-1">
      ...
      <div className="bg-gray-900/60 p-5 rounded-2xl border border-gray-800 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-gray-800/50 p-3 rounded-xl border border-gray-700/40">
            ...
            <select
              className="w-full mt-1 bg-gray-800/60 border border-gray-700/40 text-gray-100 rounded-lg px-2 py-1 text-xs font-black"
        ...
        <div className="bg-gray-800/50 p-3 rounded-xl border border-gray-700/40">
          <p className="text-[9px] font-black text-gray-400 uppercase">XP da trasferire</p>
          <div className="flex items-center gap-2 mt-1">
            <input
              type="number"
              min={0}
              className="w-full bg-gray-800/60 border border-gray-700/40 text-gray-100 rounded-lg px-2 py-1 text-xs font-black"
          ...
          {transferError && <p className="text-[10px] font-black text-rose-400 mt-2">{transferError}</p>}
          {transferOk && <p className="text-[10px] font-black text-emerald-400 mt-2">{transferOk}</p>}
        </div>
      </div>
      ...
      <div className="bg-gray-900/60 p-5 rounded-2xl border border-gray-800 space-y-3">
        ...
        >
          ...
          {showLeaderboard ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
        </button>
        {showLeaderboard && (
          <div className="space-y-1">
            {leaderboard.length === 0 ? (
              <p className="text-[10px] font-bold text-gray-400 text-center py-4">Nessuna estrazione nelle ultime 24h</p>
            ) : leaderboard.map((entry: any, i: number) => (
              <div key={entry.playerId} className="flex items-center justify-between bg-gray-800/50 px-3 py-2 rounded-xl text-[10px] font-bold">
                <div className="flex items-center gap-2">
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black ${i === 0 ? 'bg-amber-400 text-white' : i === 1 ? 'bg-gray-500 text-white' : i === 2 ? 'bg-orange-400 text-white' : 'bg-gray-700/50 text-gray-400'}`}>
                    {i + 1}
                  </span>
                  <span className="text-indigo-400">{entry.username}</span>
                  <span className="text-gray-400">Lv {entry.level}</span>
                </div>
                <span className="text-emerald-400 font-black">{entry.totalExtracted.toLocaleString()}</span>
              </div>
            ))}
          </div>
        )}
