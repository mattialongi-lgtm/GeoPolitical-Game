...
export const WarHistoryList: React.FC<WarHistoryListProps> = ({ warId }) => {
  ...
  if (loading) {
    return (
      <div className="bg-gray-900/60 p-6 rounded-2xl border border-gray-800 text-center">
        <div className="animate-spin w-6 h-6 border-2 border-gray-600 border-t-gray-300 rounded-full mx-auto" />
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-900/60 p-6 rounded-2xl border border-gray-800 space-y-4">
      <div className="flex items-center gap-3">
        <div className="p-3 rounded-2xl bg-gray-800/60 border border-gray-700/40">
          <History className="w-6 h-6 text-gray-400" />
        </div>
        <div>
          <h3 className="font-black text-white uppercase tracking-tight">Cronologia Guerra</h3>
          <p className="text-xs text-gray-400 font-medium">{history.length} eventi registrati</p>
        </div>
      </div>

      {history.length === 0 ? (
        <p className="text-center text-sm text-gray-400 py-4">Nessun evento registrato.</p>
      ) : (
        <div className="space-y-2">
          {history.map((entry) => {
            const eventConfig = EVENT_LABELS[entry.eventType] || { emoji: '📋', label: entry.eventType };
            return (
              <div key={entry.id} className="flex items-start gap-3 p-3 rounded-2xl bg-gray-800/50 hover:bg-gray-700/50 transition-colors">
                <span className="text-lg flex-shrink-0">{eventConfig.emoji}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-black text-gray-100">{eventConfig.label}</p>
