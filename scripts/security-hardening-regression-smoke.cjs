...
function run() {
  ...
  const warsLegacyRoutesTs = readFileSync('backend/routes/wars-legacy.routes.ts', 'utf8');
  const warsLegacyHandlerTs = readFileSync('backend/handlers/wars-legacy.handler.ts', 'utf8');
  const mediaHandlerTs = readFileSync('backend/handlers/media.handler.ts', 'utf8');
  const governanceHandlerTs = readFileSync('backend/handlers/governance.handler.ts', 'utf8');
  const appSource = `${serverTs}\n${warsLegacyRoutesTs}\n${warsLegacyHandlerTs}\n${mediaHandlerTs}\n${governanceHandlerTs}`;
  ...
  mustInclude(appSource, "app.get('/api/military-agreements/:stateId'", 'missing military-agreements endpoint');
  mustInclude(appSource, "app.get('/api/war-departments/:stateId'", 'missing war-departments endpoint');
  mustInclude(appSource, "app.get('/api/revolutions/:regionId'", 'missing revolutions endpoint');
  mustInclude(appSource, "app.get('/api/coups/:regionId'", 'missing coups endpoint');
  mustInclude(appSource, 'const canRead = await canReadRegionScopedData(req.user, stateId);', 'state read authorization guard missing');
  mustInclude(appSource, 'const canRead = await canReadRegionScopedData(req.user, regionId);', 'region read authorization guard missing');

  // 2) Newspaper role escalation guard (editor cannot self-escalate invites)
  mustInclude(appSource, "const targetRole = normalizeNewspaperRole(role || 'writer');", 'newspaper role normalization missing');
  mustInclude(appSource, 'if (!canAssignNewspaperRole(myMember.role, targetRole))', 'newspaper assignment guard missing');
  mustInclude(appSource, "if (actorRole === 'editor') return targetRole === 'writer';", 'editor assignment matrix missing');

  // 3) Apply flow guarded by RPC + duplicate pending DB uniqueness
  mustInclude(appSource, "supabase.rpc('create_application_atomic'", 'apply endpoint no longer uses atomic RPC');
  mustInclude(appSource, 'duplicate_pending: 409,', 'apply duplicate pending HTTP mapping missing');
  ...
  mustInclude(appSource, "supabase.rpc('resolve_application_atomic'", 'resolve endpoint no longer uses atomic RPC');
  ...
  const lobbiesGetChunk = (appSource.split("app.get('/api/lobbies/:regionId'")[1] || '').split("app.post('/api/lobbies/:id/expire'")[0] || '';
