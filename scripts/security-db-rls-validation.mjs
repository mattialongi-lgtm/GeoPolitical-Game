...
const ISO_LETTERS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
const randomIsoSuffix = (len = 3) =>
  Array.from({ length: len }, () => ISO_LETTERS[Math.floor(Math.random() * ISO_LETTERS.length)]).join('');
const regionId = `R${randomIsoSuffix(3)}`;
const otherRegionId = `S${randomIsoSuffix(3)}`;
